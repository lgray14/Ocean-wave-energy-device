...
# phase shifted trial:
# x += .4*np.sin(2*np.pi*.2*t - np.pi/3)
# real_power += 1/2*(.4**2)*(.2**2)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
for i in range(len(peak_freqs)):
    ...
    reconstructed += peak_amps[i]*np.sin(float(2*np.pi*peak_freqs[i])*np.array(t))
...
# print(f"Modelled power: {round(modelled_power, 4)}W/damping")
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# doesn't match phase but gets the amplitude and frequency excellently